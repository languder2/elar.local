<div class="container-lg">
    <div class="row">
        ...
        <div class="col col-lg-2 col-sm-4 pt-2 fs-5 text-end">
            <a href="<?=base_url('admin/years/add')?>" class="btn btn-primary">
                Создать
            </a>
        </div>
    </div>
    ...
    <?php else:?>
        <div class=" years-list-grid my-3">
            ...
            <div class="grid-title bg-primary text-white">Год</div>
            ...
            <?php foreach($list as $key=>$item):?>
                ...
                <div>
                    <div class="form-check form-switch">
                        <label>
                            <input class="form-check-input float-none change-visible" data-link="/admin/years/change-visible" data-id="<?=$item->id?>" type="checkbox" <?=$item->display?"checked":""?>>
                        </label>
                    </div>
                </div>

                <div>
                    <a class="btn btn-danger btn-sm" href="<?=base_url("admin/years/delete/$item->id")?>"><i class="bi bi-trash3"></i></a>
                </div>
            <?php endforeach;?>
        </div>
    <?php endif;?>
