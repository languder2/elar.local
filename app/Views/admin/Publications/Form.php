<div class="w-50 mx-auto">
    <form action="/admin/publications/form-processing" method="post" enctype="multipart/form-data">
        ...
        <input type="hidden" name="form[data][pdf]" value="<?=$form->data->pdf??""?>">
        <input type="hidden" name="form[data][fileName]" value="<?=$form->fileName??""?>">
        ...
        <div class="form-floating my-2 px-1">
            <input type="text" name="form[data][name]" id="form-name" placeholder="Название" value="<?=$form->data->name??""?>" required class="
                    form-control h-auto
                    <?=empty($validatorErrors->{"form.data.name"})?"":"is-invalid"?>
                "
            >
            <label class="h-auto w-auto" for="form-name">Название</label>
        </div>
        <div class="form-floating my-2 px-1">
            <input type="date" name="form[data][date]" id="form-date" placeholder="Название" value="<?=$form->data->date??date("Y-m-d")?>" required class="form-control h-auto">
        ...
        <div class="my-2 px-1">
            <label class=" form-check form-switch">
                <input
                        ...
                        type="checkbox"
                        value="1"
                        <?=empty($form->data->display)?"":"checked"?>
                >
                Опубликовать
            </label>
        </div>
        <div class="my-2 px-1">
            <input name="pdf" <?=empty($form->pdf)?"":"required"?> class="form-control" type="file" accept="application/pdf">
        </div>
        ...
        <?php endif;?>
        <div class="form-floating my-2 px-1">
            <input type="text" name="form[data][tags]" id="form-tags" placeholder="Теги" value="<?=$form->data->tags??""?>" class="form-control h-auto">
            <label class="h-auto w-auto" for="form-tags">Укажите теги, разделив запятой</label>
        </div>
        <div class="form-floating my-2 px-1">
            <textarea name="form[data][description]" id="form-description" class="form-control h-auto" rows="5" style="resize: none;" placeholder="Описание"><?=$form->data->description??""?></textarea>
            <label class="h-auto w-auto" for="form-description">Описание</label>
        </div>
        <div class="mb-3">
            <div class="my-2 px-1">
                <label for="form-section">Раздел</label>
                <select class="form-select" name="form[data][section]" size="5" required id="form-section">
        ...
        <div class="mb-3">
            <div class="my-2 px-1">
                ...
                <select class="form-select" name="form[data][source]" required id="form-source">
        ...
        <div class="mb-3">
            <div class="my-2 px-1">
                ...
                <select class="form-select" name="form[data][collections][]" multiple size="5" id="form-collections">
