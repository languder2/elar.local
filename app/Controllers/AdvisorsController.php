...
class AdvisorsController extends BaseController
{
    ...
    public function adminList(): string
    {
        ...
        $filter= [];
        if($this->session->has("advisorsFilter"))
            $filter= $this->session->get("advisorsFilter");

        $list= $this->model->db->table("advisors");
        if(!empty($filter))
        ...
        $page['filter']= view("admin/Advisors/Filter",["filter"=>(object)$filter]);

        $page['pageContent']= view("admin/Advisors/List",$page);
    ...
    public function form($action= "add",$id= false):string|RedirectResponse
    {
        ...
        $page['data']["title"] = "Научный руководитель: Добавить";
        ...
        $page['pageContent']= view("admin/Advisors/Form",$page['data']);
        return view(ADMIN."/template/page",$page);
    }
    public function formProcessing():string|RedirectResponse
    {
        ...
        if (!$inputs) {
            ...
            if($form->action=="add")
                return redirect()->to(base_url("/admin/advisors/add"));
        }
        ...
        if($form->action=="add"){
            $this->model->db->table("advisors")->insert($sql);
            $this->session->setFlashdata("message",(object)["type"=>"success","class"=>"callout-success","message"=>"Руководитель добавлен: #".$this->db->insertID().": $form->name"]);
        }

        return redirect()->to(base_url("/admin/advisors/"));
    }
    public function delete($id=false):RedirectResponse|string
    {
        ...
        $current= $this->db->table("advisors")->where(['id'=>$id])->get()->getFirstRow();

        if($current->cnt)
            $this->session->setFlashdata("message",(object)["type"=>"error","class"=>"callout-error","message"=>"Руководитель не может быть удален, т.к. содержит публикации.<br>Год #$current->id: $current->name"]);
        else{
            $this->model->db->table("authors")->delete(["id"=>$id]);
            $this->session->setFlashdata("message",(object)["type"=>"success","class"=>"callout-success","message"=>"Руководитель удален: #$current->id $current->name"]);
        }
    ...
    public function setFilter():RedirectResponse|string
    {
        ...
        $this->session->set("advisorsFilter",$filter);
        return redirect()->to(base_url("/admin/advisors/"));
    }
    ...
    public function changeVisible():bool
    {
        ...
        $this->model->db->table("advisors")->update(["display"=>$form->display],["id"=>$form->id]);
        return true;
    }
